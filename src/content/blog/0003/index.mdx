...
&\to \pqty{g(x_0) \pm \epsilon} \sqrt{\frac{2\pi}{\abs{f''(x_0) \pm \epsilon}}} \quad (\lambda \to \infty).
\end{align}
$$

　$I_{out}$ は、 $x_0$ が唯一の最大値を取る点であることから、 

$$
\exist c > 0 \quad \forall x ,\quad \abs{x-x_0} \ge \delta \implies f(x) < f(x_0) - c.
$$
...
$$
\begin{aligned}
\lim_{\lambda \to \infty} e^{-\lambda f(x_0)} \sqrt{\lambda} \int_a^b e^{\lambda f(x)} g(x) \dd{x} &= g(x_0) \sqrt{\frac{2\pi}{\abs{f''(x_0)}}} \quad (\lambda \to \infty), \\
\lim_{\lambda \to \infty} e^{-\lambda f(x_0)} \sqrt{\frac{\lambda}{2\pi}} \int_a^b e^{\lambda f(x)} g(x) \dd{x} &= g(x_0) \sqrt{\frac{1}{\abs{f''(x_0)}}} \quad (\lambda \to \infty).
\end{aligned}
$$
